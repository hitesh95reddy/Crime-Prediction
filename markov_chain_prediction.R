require("RMySQL")
con <- dbConnect(RMySQL::MySQL(), dbname = "semester6",
...
length(row)
row
tpm<- matrix(c(0),nrow = 5, ncol = 5,byrow=TRUE,
             dimnames = list(c("networks","DATA_MINING","management","Elective-1","Elective-2"),
                             c("networks","DATA_MINING","management","Elective-1","Elective-2")))
...
for (i in 1:5) tpm[i, ] <- tpm[i, ] / sum(tpm[i, ])
tpm
...
length(con_row)
con_row
con_matrix<- matrix(c(0),nrow = 1, ncol = 5,byrow=TRUE,
                    dimnames = list(c("pro"),c("networks","DATA_MINING","management","Elective-1","Elective-2")))
...
ascend<-order(result)
print(ascend)
subject[data_row,1]<-ifelse(ascend[5]==1,"networks",
                 ifelse(ascend[5]==2,"DATA_MINING",
                        ifelse(ascend[5]==3,"management",
                                                ifelse(ascend[5]==4,"Elective-1",
                                                       "Elective-2"))))
subject[data_row,2]<-ifelse(ascend[4]==1,"networks",
                 ifelse(ascend[4]==2,"DATA_MINING",
                        ifelse(ascend[4]==3,"management",
                               ifelse(ascend[4]==4,"Elective-1",
                                      "Elective-2"))))
}
subject
